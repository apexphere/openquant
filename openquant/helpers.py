...
import numpy as np
import base64
from openquant.constants import CANDLE_SOURCE_MAPPING
from openquant.constants import TIMEFRAME_PRIORITY
from openquant.constants import SUPPORTED_COLORS
...
def app_currency() -> str:
    from openquant.info import exchange_info
...
def get_strategy_class(strategy_name: str):
    ...
    # main framework
        ...
        strategy_dir = f'openquant.strategies.{strategy_name}.{strategy_name}'
...
def opposite_side(s: str) -> str:
    from openquant.enums import sides
...
def opposite_type(t: str) -> str:
    from openquant.enums import trade_types
...
def quote_asset(symbol: str) -> str:
    try:
        ...
        from openquant.exceptions import InvalidRoutes
...
def side_to_type(s: str) -> str:
    from openquant.enums import trade_types, sides
...
def validate_response(response):
    if response.status_code != 200:
        err_msg = f"[{response.status_code}]: {response.json()['message']}\nPlease contact us at support@openquant.trade if this is unexpected."
...
def timeframe_to_one_minutes(timeframe: str) -> int:
    from openquant.utils import timeframe_to_one_minutes
...
def validate_cwd() -> None:
    ...
    if not is_jesse_project():
        print(
            color(
                'Current directory is not a Jesse project. You must run commands from the root of a Jesse project. Read this page for more info: https://docs.openquant.trade/docs/getting-started/#create-a-new-jesse-project',
                'red'
            )
        )
        os._exit(1)
