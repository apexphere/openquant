...
def _save_lsp_version(lsp_version: str) -> None:
    ...
    """
    from openquant import JESSE_DIR
...
def run_lsp_server():
    ...
    if not os.path.exists(lsp_dir):
        raise Exception("LSP directory not found. Please re-install it first by restarting the openquant.")
    
    if not os.path.exists(start_script):
        raise Exception("Python Language Server start script not found. Please re-install it first by restarting the openquant.")
