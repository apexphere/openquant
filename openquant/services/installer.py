...
import click
import os
from openquant.info import JESSE_API_URL
...
def install(is_live_plugin_already_installed: bool, strict: bool):
    if is_live_plugin_already_installed:
        from openquant_live.version import __version__
        click.clear()
    ...
    if not ENV_VALUES['LICENSE_API_TOKEN']:
        if strict:
            print("No license API token set. Please set the LICENSE_API_TOKEN environment variable to continue. If you don't have one yet, create one at https://openquant.trade/user/api-tokens" )
        return
    else:
    ...
    if is_arm and (os_name in ['linux', 'windows']):
        raise NotImplementedError(f'ARM versions of {os_name} are not supported. If you need them, send a request to contact@openquant.trade')
    ...
    from openquant.version import __version__ as jesse_version
    ...
    try:
        ...
        response = requests.post(
            'https://api1.openquant.trade/api/download-release',
            headers={'Authorization': 'Bearer ' + access_token},
            params={
                'os': formatted_os_name,
                'python_version': '{}.{}'.format(*jh.python_version()),
