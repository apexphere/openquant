...
from openquant.services.web import (
...
from openquant.info import JESSE_API2_URL
...
def make_strategy(json_request: NewStrategyRequestJson, authorization: Optional[str] = Header(None)) -> JSONResponse:
    ...
    from openquant.services import strategy_handler
...
async def index_jesse_trade_strategies(
    ...
    """
    Browse strategies from openquant.trade
    """
    if not authenticator.is_valid_token(authorization):
        return authenticator.unauthorized_response()
    ...
    try:
        ...
        return JSONResponse({
            'status': 'error',
            'message': f'Error connecting to openquant.trade: {str(e)}'
        },         status_code=500)
...
async def get_jesse_trade_periods(
    ...
    """
    Get available trading periods from openquant.trade
    """
    if not authenticator.is_valid_token(authorization):
        return authenticator.unauthorized_response()
...
async def get_jesse_trade_strategy(
    ...
    """
    Get a specific strategy from openquant.trade
    """
    if not authenticator.is_valid_token(authorization):
        return authenticator.unauthorized_response()
...
async def get_jesse_trade_strategy_metrics(
    ...
    """
    Get metrics for a specific strategy from openquant.trade
    """
    if not authenticator.is_valid_token(authorization):
        return authenticator.unauthorized_response()
...
async def import_strategy(
    ...
    """
    Import a strategy from openquant.trade
    """
    if not authenticator.is_valid_token(authorization):
        return authenticator.unauthorized_response()

    try:
        # Fetch the strategy from openquant.trade
        headers = {}
        if jesse_trade_token:
            headers['Authorization'] = f'Bearer {jesse_trade_token}'
